...
for _ in range(num_runs):
    ...
    early_stopping = EarlyStoppingAtLoss(monitor='loss', value=90, verbose=1)
    history = model.fit(traindata, trainlabel, epochs=700, batch_size=batch_size,callbacks=[early_stopping])  # Training model
...
print('PCMLP Training has been finished!')
print('The Total PCMLP Training Time is : ', round(Training_time, 6), ' seconds')
# endregion
...
print("MAE PCMLP Train TCO:", mae_MLP_Train_TCO)
print("MAE PCMLP Train P:", mae_MLP_Train_P)
print("RMSE PCMLP Train TCO:", rmse_MLP_Train_TCO)
print("RMSE PCMLP Train P:", rmse_MLP_Train_P)
print("R^2 PCMLP Train TCO:", r2_MLP_Train_TCO)
print("R^2 PCMLP Train P:", r2_MLP_Train_P)
print("MAPE PCMLP Train TCO:", mape_MLP_Train_TCO)
print("MAPE PCMLP Train P:", mape_MLP_Train_P)
print("MSE PCMLP Train TCO:", mse_MLP_Train_TCO)
print("MSE PCMLP Train P:", mse_MLP_Train_P)
print("loss_energy PCMLP Train:", loss_energy_MLP_Train)
print("loss PCMLP Train:", loss_MLP_Train)
COP_train = 6330 * dataset2832[:, 8] / y_train[:, 1]
COP_train = pd.DataFrame(COP_train)
#COP_train.to_csv('NETOUT_TRAIN_PCMLP_COP_fp_1.csv')
best_y_train = pd.DataFrame(y_train)
#best_y_train.to_csv('NETOUT_TRAIN_PCMLP_TCO_P_fp_1.csv')
...
error_Test.to_csv('NETOUT_TEST_PCMLP_error_fp_1.csv')
print("Testing accuracy indicators")
print("MAE PCMLP Test TCO:", mae_MLP_Test_TCO)
print("MAE PCMLP Test P:", mae_MLP_Test_P)
print("RMSE PCMLP Test TCO:", rmse_MLP_Test_TCO)
print("RMSE PCMLP Test P:", rmse_MLP_Test_P)
print("R^2 PCMLP Test TCO:", r2_MLP_Test_TCO)
print("R^2 PCMLP Test P:", r2_MLP_Test_P)
print("MAPE PCMLP Test TCO:", mape_MLP_Test_TCO)
print("MAPE PCMLP Test P:", mape_MLP_Test_P)
print("MSE PCMLP Test TCO:", mse_MLP_Test_TCO)
print("MSE PCMLP Test P:", mse_MLP_Test_P)
print("loss_energy PCMLP Test:", loss_energy_MLP_Test)
print("loss PCMLP Test:", loss_MLP_Test)
COP_test = 6330 * dataset7_30[:, 8] / y_pred[:, 1]
COP_test = pd.DataFrame(COP_test)
#COP_test.to_csv('NETOUT_TEST_PCMLP_COP_fp_1.csv')
y_pred = pd.DataFrame(y_pred)
#y_pred.to_csv('NETOUT_TEST_PCMLP_TCO_P_fp_1.csv')
...
plt.title('PCMLP Training Loss vs Epochs')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.grid(True)
plt.legend()
...
train_losses.to_csv('train_losses_PCMLP.csv')
